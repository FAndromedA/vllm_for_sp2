...
def sse_chunk_gated_delta_rule_fwd_kernel_h_blockdim64(
    k,
    v,
    w,
    v_new,
    ...
    i_n, i_h = i_nh // H, i_nh % H # batch sequence index and head index
    if IS_VARLEN:
        bos, eos = (
...
def sse_chunk_gated_delta_rule_fwd_h(
    k: torch.Tensor,
    w: torch.Tensor,
    u: torch.Tensor,
    ...
    H = u.shape[-2] # number of heads in the value tensor
    BT = chunk_size
    ...
    sse_chunk_gated_delta_rule_fwd_kernel_h_blockdim64[grid](
        k=k,
        v=u,
        w=w,
        v_new=v_new,
