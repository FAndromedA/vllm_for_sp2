...
from .attn_hf import Attention
from .sse_hf import SSEGLA, SSEGDN
from .moba_attn_hf import MoBAAttention
from .sse_swa_hf import SSEGLAH, SSEGDNH
...
class SSESWAMoBABlock(GradientCheckpointingLayer):

    def __init__(self, config: SSESWAMoBAConfig, layer_idx: int):
        super().__init__()
        ...
        if config.attn is not None and layer_idx in config.attn['layers']:
            ...
            # Linear & Window Attention Layers
    ...
    def forward(
        ...
        **kwargs: Unpack[dict],
        ...
        # used for attention-output-based KD
...
class SSESWAMoBAPreTrainedModel(PreTrainedModel):

    config_class = SSESWAMoBAConfig
    base_model_prefix = 'model'
    supports_gradient_checkpointing = True
    _no_split_modules = ['SSESWAMoBABlock']
    ...
    def _init_weights(
        ...
        if isinstance(module, SSEGDNH) and next(module.parameters()).device.type != 'meta':
            with torch.no_grad():
...
class SSESWAMoBAModel(SSESWAMoBAPreTrainedModel):
    ...
    def __init__(self, config: SSESWAMoBAConfig):
        ...
        self.layers = nn.ModuleList([SSESWAMoBABlock(config, layer_idx) for layer_idx in range(config.num_hidden_layers)])
    ...
    def forward(
        ...
        **kwargs: Unpack[dict],
        ...
        #     warnings.warn("`SSESWAMoBAModel` does not `output_attentions` now, setting it to `False`.")
        #     output_attentions = False
...
class SSESWAMoBAForCausalLM(SSESWAMoBAPreTrainedModel, FLAGenerationMixin):

    _tied_weights_keys = ["lm_head.weight"]

    def __init__(self, config):
        super().__init__(config)
        self.model = SSESWAMoBAModel(config)
        self.vocab_size = config.vocab_size
