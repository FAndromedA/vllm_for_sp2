...
class MobaSseFlashAttentionBackend(AttentionBackend):
    ...
    @staticmethod
    def get_impl_cls() -> type["MobaSseFlashAttentionImpl"]:
        return MobaSseFlashAttentionImpl

    @staticmethod
    def get_builder_cls() -> type["MobaSseFlashAttentionMetadataBuilder"]:
        return MobaSseFlashAttentionMetadataBuilder
...
@dataclass
class MobaSseFlashAttentionMetadata:
...
def _get_sliding_window_configs(
    ...
    for layer in layers.values():
        assert isinstance(layer.impl, MobaSseFlashAttentionImpl)
        sliding_window_configs.add(layer.impl.sliding_window)
    return sliding_window_configs


class MobaSseFlashAttentionMetadataBuilder(AttentionMetadataBuilder[MobaSseFlashAttentionMetadata]):
    # FA3:
    ...
    def __init__(
        ...
        self.spec_squence_masks = torch.empty(
            (self.decode_cudagraph_max_bs, ),
            dtype=torch.bool,
            device=device,
        )
    ...
    def build(
        ...
        def schedule(
            ...
            if cache_dtype.startswith("fp8"):
                qkv_dtype = MobaSseFlashAttentionBackend.get_fp8_dtype_for_flashattn(
                    cache_dtype
                )
            else:
        ...
        # =====| from GDNAttentionMetadataBuilder starts |=====
        # =====| from GDNAttentionMetadataBuilder starts |=====
        ...
        if spec_sequence_masks is None:
            ...
            non_spec_state_indices_tensor = m.block_table_tensor[:, 0] # shape 
        ...
        # =====| from GDNAttentionMetadataBuilder ends | =====
        # =====| from GDNAttentionMetadataBuilder ends | =====


        attn_metadata = MobaSseFlashAttentionMetadata(
...
class MobaSseFlashAttentionImpl(AttentionImpl):
    ...
    def forward(
        ...
        if self.kv_cache_dtype.startswith("fp8"):
            ...
            dtype = MobaSseFlashAttentionBackend.get_fp8_dtype_for_flashattn(
                self.kv_cache_dtype
            )
            key_cache = key_cache.view(dtype)
            value_cache = value_cache.view(dtype)

        if (not attn_metadata.use_cascade) or (self.is_moba == True):
